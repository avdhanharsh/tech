...
if (isset($_POST['submit'])) {
    ...
    if ($thumb_size > 2000000) {
        ...
        $add_papersection = $conn->prepare("INSERT INTO `paper` (id, tutor_id, title, pdf, thumb, status) VALUES (?,?,?,?,?,?)");
        $add_papersection->execute([$id, $tutor_id, $title, $rename_pdf, $rename_thumb, $status]);
        move_uploaded_file($thumb_tmp_name, $thumb_folder);
        move_uploaded_file($pdf_tmp_name, $pdf_folder);
        $message[] = 'New paper uploaded!';
    }
}
...
<section class="video-form">
    ...
    <h1 class="heading">Upload Paper</h1>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <p>Select Paper (PDF) <span>*</span></p>
        <input type="file" name="video" accept=".pdf" required class="box">
        <input type="submit" value="Upload Paper" name="submit" class="btn">
    </form>
