...
if(isset($_POST['update'])){
   ...
   $update_book = $conn->prepare("UPDATE `books` SET title = ?, status = ? WHERE id = ?");
   $update_book->execute([$title, $status, $book_id]);
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         $update_thumb = $conn->prepare("UPDATE `books` SET thumb = ? WHERE id = ?");
         $update_thumb->execute([$rename_thumb, $book_id]);
...
$pdf_folder = '../uploaded_files/'.$rename_pdf;
   ...
   if(!empty($pdf)){
      $update_pdf = $conn->prepare("UPDATE `books` SET pdf = ? WHERE id = ?");
      $update_pdf->execute([$rename_pdf, $book_id]);
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="title" maxlength="100" required placeholder="enter Book title" class="box" value="<?= $fecth_pdf['title']; ?>">
      ...
      <div class="flex-btn">
         <a href="view_content.php?get_id=<?= $paper_id; ?>" class="option-btn">view Book</a>
