...
if (isset($_POST['delete_pdf'])) {
    ...
    $verify_pdf = $conn->prepare("SELECT * FROM `books` WHERE id = ? LIMIT 1");
    $verify_pdf->execute([$delete_id]);

    if ($verify_pdf->rowCount() > 0) {
        $delete_pdf_info = $conn->prepare("SELECT thumb, pdf FROM `books` WHERE id = ? LIMIT 1");
        ...
        $delete_pdf = $conn->prepare("DELETE FROM `books` WHERE id = ?");
...
<section class="contents">
   ...
   <!-- <div class="box" style="text-align: center;">
      <h3 class="title" style="margin-bottom: .5rem;">create new Books</h3>
      <a href="add_book.php" class="btn">add Books</a>
   </div> -->

   <?php
      $select_pdf = $conn->prepare("SELECT * FROM `books` WHERE tutor_id = ? ORDER BY date DESC");
   ...
   ?>
      <div class="box">
         ...
         <!-- <form action="" method="post" class="flex-btn">
            ...
            <a href="update_book.php?get_id=<?= $pdf_id; ?>" class="option-btn">update</a>
            <input type="submit" value="delete" class="delete-btn" onclick="return confirm('delete this pdf?');" name="delete_pdf">
         </form> -->
         <a href="view_book_student.php?get_id=<?= $pdf_id; ?>" class="btn" target = "_blank">view Book</a>
      </div>
   <?php
         }
      }else{
